...
```{r}
norm_df
mo_tbl = norm_df %>% group_by(Symbol, day_in_mo) %>% summarise(mo_ret=mean(day_ret))
mo_tbl
na.omit(norm_mo_tbl)
# library(reshape2)
...
ttl_by_mo = mo_tbl %>% group_by(day_in_mo) %>% summarise(agg_monthly_return = mean(mo_ret, na.rm=T))
ttl_by_mo
agg_monthly <- ggplot(data = ttl_by_mo, aes(x=day_in_mo, y=agg_monthly_return)) +geom_line(aes(group = 1))
agg_monthly
...
norm_df
qtr_tbl = norm_df %>% group_by(Symbol, day_into_qtr) %>% summarise(qtr_ret=mean(day_ret))
qtr_tbl
na.omit(qtr_tbl)
qtr_tbl
# library(reshape2)
...
ttl_by_qtr = qtr_tbl %>% group_by(day_into_qtr) %>% summarise(agg_quarterly_return = mean(qtr_ret, na.rm=T))
ttl_by_qtr
